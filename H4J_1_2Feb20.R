...
# to set or change your working directory use setwd("~Desktop/R/example.csv")  
...
df.sent <- df.sent[ , c(1:3,6,10,14:22,25:28,32,39)] # keep all rows with blank space, delete columns after comma
...
sub.sent <- sub.sent[ , c(1:2,4:11,14,18:21)]


# 5) Why are there 34153 duplicated rows in the defendant subset data frame?
...
df.charcount <- as.data.frame(table(df.sent$offense_title))
...
barplot(sub.top5char$Freq, # barplot is useful for categorical variables like charge.
	...
	names.arg = c("PCS","DUI", "Ret","No Lic","UUW","Theft"), ylab = "Frequency")  #Note here how to add labels!
...
# e.g. age
class(df.sent$age_at_incident)
hist(df.sent$age_at_incident) #histogram reveals nonsensical values here

df.sent$age_at_incident <- ifelse(df.sent$age_at_incident>86, NA, df.sent$age_at_incident) #replace over 86 ages with NA

table(is.na(df.sent$age_at_incident)) # 2952 NA values in this column
...
# to really fix this, you would have to use string manipulation, for now lets just use the most common values like with offense title
...
table(is.na(sub.prison$commitment_term)) # 336 NA values
